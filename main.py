...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while len(arcs):
            ...
            if failure:
                return failure
            assignCol, failure = self.remove_domain_column(grid,cell[0], cell[1])
            if failure:
                return failure
    ...
    def pre_process_consistency(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) == 1:
                    arcConsistentList.append((i, j))
    
        self.consistency(grid, arcConsistentList)    
class Backtracking:
    """
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            ...
            fail = ac.consistency(copy_g,var)
            if not fail:
...
for p in problems:
    ...
    interval = end-start
    runTimeFA.append(interval)
    ##temp[0].print()
    #print("Is the current grid a solution?", temp[0].is_solved())
...
plotter = PlotResults()
plotter.plot_results(runTimeMRV, runTimeFA, "Running Time Backtracking (MRV)", "Running Time Backtracking (FA)", "running_time")    